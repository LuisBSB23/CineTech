...
export default function Profile() {
  ...
  const handleCancelPurchaseClick = (reservaId: number) => {
      setPurchaseToCancel(reservaId);
  };

  const handleActuallyCancelPurchase = async () => {
      if (!purchaseToCancel) return;
      const toastId = toast.loading("Cancelando...");
      try {
          await cancelarReservaApi(purchaseToCancel);
        ...
        {purchaseToCancel && (
            ...
            >
                ...
                >
                    ...
                    <h3 className="text-xl font-bold text-white text-center mb-2">Cancelar Compra?</h3>
                    <p className="text-slate-400 text-center text-sm mb-6">
                        Os assentos serão liberados imediatamente.
                    </p>
                    <div className="flex gap-3">
                        <Button variant="secondary" onClick={() => setPurchaseToCancel(null)} className="flex-1">
                            Não, Manter
                        </Button>
                        <Button 
                            onClick={handleActuallyCancelPurchase} 
                            className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-white shadow-yellow-900/20"
                        >
                            Sim, Cancelar
                        </Button>
                    </div>
